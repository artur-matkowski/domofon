...
| `receivers/GeofenceReceiver` + `ArrivalFlow` | Validate the ENTER event; run the arrival pop-up; the shared arrival cooldown |
...
| `domain/HomeFenceCrossing` | The inward-crossing rule, as a pure function |
...
- On ENTER: `ArrivalFlow` takes a `ConnectionLease("arrival")` via `use {}`, waits
  ...
  `appScope`.
- **Both triggers funnel through `ArrivalFlow`, which enforces a 10-minute cooldown.** They
  will routinely both notice one approach seconds apart; one pop-up per arrival is the point.
  The timestamp is *persisted*, because the native fence delivers into a process that is
...
- Every outcome is recorded to `GeofenceStatusStore` and rendered in Settings: the
  ...
  delivery, the last in-app crossing, the last pop-up and which trigger won it, and the reason
  any delivery was thrown away. **Timestamps and status codes only — never coordinates**
...
3. **No `INITIAL_TRIGGER_ENTER`, and the in-app fence's first reading never fires either.**
   ...
   continuous (tracker stopped, radius edited).
